...
|------|------|------|
| Node 版本 | >= 18 | |
| 环境变量 | 从 .env.example 复制到 .env.local | |

## 踩坑记录
- **Java 子模块构建顺序**: 必须先 `mvn install -pl common`。原因：其他模块依赖 common SNAPSHOT
```

### 条目上限

模块地图 ≤ 20 | 领域映射 ≤ 40 | 构建命令 ≤ 10 | 环境条件 ≤ 15 | 踩坑 ≤ 15

### 写入规则

每次任务完成后（Phase 3.6 之后），检查 4 个触发条件：
- 新模块发现 → 追加到"模块地图"
- 新领域→文件映射 → 追加到"领域映射"
- 新 build/test 命令 → 更新"构建 & 测试"
- 非显而易见的问题 → 追加到"踩坑记录"

**去重**（主键匹配）：
- 模块地图：目录路径为主键。主键相同 → 更新，不追加
- 领域映射：领域名为主键
- 踩坑记录：问题描述前 20 字为主键

**与 CLAUDE.md 的边界**：DEVIN.md = Devin 积累的补充知识。写入前读 CLAUDE.md，已有的不重复写入。
...
- **局部清理**（每次任务后）：只检查本次涉及的分区条目，引用的文件不存在则删除
- **全量清理**（用户说"整理 DEVIN.md"或局部清理发现 ≥5 条中 >30% 过时时触发）

### 迁移

检测到非结构化 DEVIN.md（缺少 3+ 个标准分区标题）时：
...
任务计数追踪：在 `.devin-state.json` 的 `consolidation` 字段中记录 `{"lastConsolidation": "日期", "tasksSinceConsolidation": N}`。每次任务完成（phase → "done"）时 `tasksSinceConsolidation += 1`。整合后重置为 0。不存储在 DEVIN.md 中（避免污染人类可读文档）。
